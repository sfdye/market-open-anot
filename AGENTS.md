...
- The build number is the `BUILD` constant in `app.config.ts`, feeding iOS `buildNumber` and Android `versionCode` from one value so the two cannot drift; `appVersionSource` is `local` and no profile sets `autoIncrement`. It is applied *before* the `APP_VARIANT` branch, so anything added to the dev override must spread `base.ios`/`base.android`, never `config.ios`/`config.android`, or that variant silently loses it. Bump with `npm run release`, never by hand — it tags the commit, and EAS builds committed state, so an unbumped commit re-sends a number the store has already taken.
