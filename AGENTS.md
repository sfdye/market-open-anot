...
- The map ends where OneMap's tiles do, `SG_BOUNDS` in `lib/core/map-bounds.ts`. Outside it OneMap answers a tile request with something that is not a PNG, which MapLibre logs at error level and the bridge turns into a `console.error` — a LogBox red box, silenced in dev only by `configureMapLogging()` in `lib/maplibre.ts` (`LogManager` holds one handler, so a second caller replaces it rather than adding to it). Keeping the camera inside the box takes two things, because `maxBounds` constrains the *centre* and not the viewport: the box handed to it is inset by half the span the map last reported (`centerLimit`), and `onRegionDidChange` eases the centre back in after a zoom-out that outgrew it. Reach for the source's `bounds` and that inset pair together — either alone leaves empty background reachable.
