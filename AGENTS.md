...
- Which OneMap basemap is drawn is a stored preference, `state.basemapPref` resolved by `resolveBasemap` in `lib/core/basemap.ts` — the same choice/result split as `langPref` with one difference: the result is *not* derived in `setState`, because `'system'` resolves against `theme.scheme`, which is `useColorScheme` and not store state. So it resolves in `MarketMap`, where `MAP_STYLES` is keyed by basemap and built once at module scope (a fresh style object reloads every tile), and the colour showing through the tile seams follows the basemap rather than the app's appearance. Three of OneMap's six are offered; adding one is an entry in `BASEMAPS`, in `MAP_STYLES`, and in `MAP_STYLE_LABELS` on the settings screen — each a `Record<Basemap, …>`, so a forgotten one fails typecheck. Every style carries SLA's "INSET NOT TO SCALE" box in the sea south-west of the island; it is baked into their raster tiles and no style option removes it.
