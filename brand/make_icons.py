...
BRAND_GREEN_TOP = (36, 108, 48)
BRAND_GREEN = (46, 125, 50)  # #2e7d32 — the app's canonical green
WHITE = (255, 255, 255)
...
def layered_ground():
    """A deliberately restrained surface gradient, leaving the approved mark untouched."""
    out = Image.new('RGB', (CANVAS, CANVAS))
    draw = ImageDraw.Draw(out)
    for y in range(CANVAS):
        ratio = y / (CANVAS - 1)
        rgb = tuple(round(top + (bottom - top) * ratio)
                    for top, bottom in zip(BRAND_GREEN_TOP, BRAND_GREEN))
        draw.line((0, y, CANVAS, y), fill=rgb)
    return out


def on_ground(base, matte):
    """Keep the original white mark exactly intact over the gently layered ground."""
...
def main():
    ...
    # The mark's geometry remains the approved original; only its supporting surface changes.
    background = layered_ground()
    adaptive = scaled(matte, factor)
    flat = on_ground(background, matte)

    save(flat, 'icon.png')                                # ios.icon.light + Android legacy
    ...
    save(on_alpha(adaptive), 'adaptive-icon.png')          # Android foreground
    save(background, 'adaptive-background.png')            # Android adaptive background
    save(on_alpha(adaptive), 'monochrome-icon.png')        # Android 13+ themed icon
    flat.convert('RGBA').resize((PLAY_SIZE, PLAY_SIZE), Image.LANCZOS).save(PLAY_ICON, optimize=True)
