import { getUpcomingClosures } from "./market-logic.js";
...
export function sgInstant(civil, hour) {
...
export function shortName(rawName) {
 ...
 */
export function groupClosuresByDate(favorites, markets, today) {
    ...
    for (const favorite of favorites) {
        ...
        for (const closure of getUpcomingClosures(market, HORIZON_DAYS, today)) {
            ...
            if (!group.reasons.includes(closure.reason))
                group.reasons.push(closure.reason);
        }
    }
    return [...groups.values()].sort((a, b) => a.date.getTime() - b.date.getTime());
}
/** Bilingual copy carried over from worker/src/index.ts, with a maintenance variant. */
export function notificationCopy(group, isToday, lang) {
 ...
 */
export function buildSchedule(favorites, markets, lang, now) {
