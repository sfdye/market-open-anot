...
const strings = {
  en: {
    ...
    dataSource:
      'Data from <a href="https://data.gov.sg/datasets/d_bda4baa634dd1cc7a6c7cad5f19e2d68/view">NEA</a>',
  ...
  zh: {
    ...
    foodStalls: '个熟食摊位',
    dataSource:
      '数据来源：<a href="https://data.gov.sg/datasets/d_bda4baa634dd1cc7a6c7cad5f19e2d68/view">国家环境局 (NEA)</a>',
    lastUpdated: '最后更新：',
    addMarkets: '+ 添加巴刹',
...
function getUpcomingClosures(market: Market, days: number): Closure[] {
...
function getDisplayName(parsed: ParsedMarketName): string {
  if (lang === 'zh') {
...
function loadCachedData(): Market[] | null {
  try {
    const stored = localStorage.getItem(STORAGE.data);
    return stored ? (JSON.parse(stored) as Market[]) : null;
  } catch {
    return null;
  }
}

function saveCachedData(data: Market[]): void {
...
function fetchMarkets(callback: () => void): void {
...
function fetchFromAPI(callback: () => void): void {
  const xhr = new XMLHttpRequest();
  xhr.open('GET', API_URL);
  xhr.onload = () => {
    if (xhr.status === 200) {
      try {
        const response = JSON.parse(xhr.responseText) as { result?: { records?: Market[] } };
...
function closureReasonShort(reason: ClosureReason, remarks: string | undefined): string {
...
function removeMarket(name: string | null): void {
...
function getMarketDistance(market: Market): number | null {
...
function requestGeolocation(): void {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      renderMarketList(el<HTMLInputElement>('search-input').value);
    },
    () => {},
...
function renderPickerScreen(): void {
  ...
  el<HTMLInputElement>('search-input').placeholder = t('search');
...
function renderMarketList(query: string): void {
  ...
  list.querySelectorAll('.market-list-item').forEach((item) => {
    item.addEventListener('click', () => {
      ...
      if (star) star.textContent = favorites.indexOf(name) !== -1 ? '★' : '☆';
      updateDoneButton();
    });
  });
}
...
function initMap(): void {
  ...
  allMarkets.forEach((market) => {
    ...
    function popupHtml(fav: boolean): string {
      return (
...
function findMarket(name: string): Market | null {
...
function init(): void {
  ...
  el<HTMLInputElement>('search-input').addEventListener('input', (e) => {
    renderMarketList((e.target as HTMLInputElement).value);
  });
  ...
  const reminderBtn = el<HTMLButtonElement>('reminder-btn');
...
function setReminderBusyUI(busy: boolean): void {
  const btn = el<HTMLButtonElement>('reminder-btn');
  const enableBtn = el<HTMLButtonElement>('reminder-enable-btn');
  const dismissBtn = el<HTMLButtonElement>('reminder-dismiss-btn');
  btn.disabled = busy;
...
function renderReminderCard(): void {
...
function updateReminderBtn(): void {
