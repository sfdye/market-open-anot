...
function setPushEnabled(val: boolean): void {
...
async function syncSubscription(subscription?: PushSubscription | null): Promise<void> {
  let sub = subscription;
  if (!sub) {
    const reg = await navigator.serviceWorker.ready;
    sub = await reg.pushManager.getSubscription();
  }
  if (!sub) throw new Error('No push subscription available');

  let favorites: string[] = [];
  try {
    favorites = JSON.parse(localStorage.getItem('moa_favorites') || '[]') as string[];
  } catch {
...
export async function onFavoritesChanged(): Promise<void> {
